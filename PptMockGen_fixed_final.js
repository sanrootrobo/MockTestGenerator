...
8. **Expanation and Answer Section**
  * The  must provide reasoning in **step-by-step bullet points** (e.g., Step 1, Step 2, Step 3).
  * Steps of answer should be seperated by /n, and should use correct format.
...
// --- JSON TO PPTX CONVERSION (OPTIMIZED FOR DI) ---
...
// Check if a section is Data Interpretation related
function isDISection(sectionTitle) {
    const diKeywords = ['data interpretation', 'di', 'graph', 'chart', 'table', 'diagram'];
    return diKeywords.some(keyword => sectionTitle.toLowerCase().includes(keyword.toLowerCase()));
}

// OPTIMIZED: Create question slide with better DI layout
function createQuestionSlide(pptx, question, directions, bgImagePath, sectionTitle = '') {
    const slide = addSlideWithBackground(pptx, bgImagePath);
    const isDI = isDISection(sectionTitle);
    ...
    // REMOVED: Skip directions for DI sections to save space
    if (!isDI && directions) {
    ...
    // Check if question has SVG (likely a DI question with chart/graph)
    const hasQuestionSvg = question.svg && svgToBase64(question.svg);
    
    if (hasQuestionSvg) {
        // DI Layout: Question text at top, SVG takes most space
        const questionTextHeight = 0.8;
        ...
        // Large SVG for DI charts/graphs - optimized dimensions
        slide.addImage({ 
            data: hasQuestionSvg, 
            x: 1, y: currentY, w: 8, h: 4.5  // Larger size for better visibility
        });
        
        // Note: Options will be on next slide for DI questions
        return slide;
        ...
        // Add options directly on same slide for non-DI questions
...
// NEW: Create separate options slide for DI questions
function createDIOptionsSlide(pptx, question, bgImagePath) {
    const slide = addSlideWithBackground(pptx, bgImagePath);
    
    // Title indicating this is options for the previous question
    slide.addText(`Question ${question.questionNumber} - Options`, { 
        x: 0.5, y: 0.4, w: '90%', fontSize: 24, bold: true, color: '1A365D' 
    });
    
    let currentY = 1.2;
    
    // Add options with optimal spacing
    question.options.forEach((opt, index) => {
        const optionText = `${opt.label}) ${opt.text || ''}`;
...
function createAnswerSlide(pptx, question, bgImagePath) {
    ...
    // Check if we have step-by-step solution
    const hasSteps = question.solution.steps && question.solution.steps.length > 0;
    ...
    if (hasSteps) {
        ...
        const maxSteps = hasSolutionSvg ? 4 : 6; // Fewer steps if we have an image

        // Show steps
        question.solution.steps.slice(0, maxSteps).forEach((step, index) => {
            const stepText = `Step ${index + 1}: ${step.description || step}`;
        ...
        // If there are more steps than we can show
        if (question.solution.steps.length > maxSteps) {
            slide.addText(`... and ${question.solution.steps.length - maxSteps} more steps`, {
                x: hasSolutionSvg ? 0.5 : 0.5,
                y: currentY,
                w: hasSolutionSvg ? '45%' : '90%',
                h: 0.3,
        ...
        // Fallback to original explanation-based layout
        ...
        if (hasSolutionSvg) {
            // Split layout: explanation on left, diagram on right
...
// Helper function to break down explanation into steps if steps aren't provided
...
// Enhanced function that tries to create steps from explanation if not provided
function createAnswerSlideWithSteps(pptx, question, bgImagePath) {
    const slide = addSlideWithBackground(pptx, bgImagePath);
    ...
    // Get steps - either from question.solution.steps or parse from explanation
...
async function generatePptFromJson(jsonData, outputPath, backgroundPath) {
    try {
        console.log('📊 Creating PowerPoint presentation with optimized DI layout...');
        ...
        jsonData.sections.forEach(section => {
            ...
            section.questionSets.forEach(qSet => {
                const directions = (qSet.type === 'group' && !isDI) ? qSet.directions : null;
                
                qSet.questions.forEach(q => {
                    // Create main question slide
                    createQuestionSlide(pptx, q, directions, backgroundPath, sectionTitle);
                    
                    // For DI questions with SVG, create separate options slide
                    if (isDI && q.svg && svgToBase64(q.svg)) {
                        createDIOptionsSlide(pptx, q, backgroundPath);
                    }
                });
            });
        });
        ...
        console.log(`📊 PowerPoint generated successfully with optimized DI layout: ${path.basename(outputPath)}`);
...
function createGenerationConfig(options, model) {
    ...
    if (options.maxTokens && options.maxTokens !== 90192) {
...
async function main() {
    program
        ...
        .option("--temperature <number>", "Temperature for response generation (0.0-2.0, default: 1)", parseFloat, 0.7)
    ...
    try {
        ...
        console.log(`\n🎉 Successfully generated ${successful.length} mock test(s) with optimized DI layout!`);
