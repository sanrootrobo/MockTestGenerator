...
const systemPrompt  = `You are an expert Data Interpretation exam designer for competitive entrance exams. Your task is to generate a BRAND NEW mock test as a single, valid JSON object.
...
async function generatePowerPoint(examData, outputPath, backgroundPath = null) {
    try {
        ...
        for (const questionSet of questionSets) {
            ...
            dataSlide.addText(questionSet.setTitle || `Set ${questionSet.setNumber}`, {
...
async function generateMockTest(contents, outputPath, mockNumber, totalMocks, apiKeyManager, options = {}) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        ...
        try {
            ...
            // Corrected instantiation to use GoogleGenerativeAI
...
async function main() {
    ...
    try {
        console.log('DI Mock Test Generator v2.2.0');
        console.log('============================================');
        ...
        if (successCount < options.numberOfMocks) {
            ...
            results.filter(r => !r.success).forEach(r => {
                console.log(`  - Mock ${r.mockNumber}: ${r.error.message}`);
            });
        }
