...
1.  **Template Analysis (CRITICAL - FIRST PRIORITY):**
    ...
    *   You will create questions that CLOSELY MIRROR the template structure but also make the question as eloborate and descriptive as possible.
...
2.  **Question Generation Strategy:**
    *   For EACH question in the template, create a corresponding question with:
        ...
        - BUT MAKE QUESTION MORE ELOBORATE THEN ORIGINAL
...
1.  **Template Analysis (CRITICAL - FIRST PRIORITY):**
    ...
    *   You MUST analyze this template file thoroughly to understand:
        ...
        - The difficulty level and complexity
        - The solution explanation format
    *   You will create questions that CLOSELY MIRROR the template structure while making them significantly MORE ELABORATE and DESCRIPTIVE.

2.  **Question Generation Strategy - ENHANCED ELABORATION:**
    *   For EACH question in the template, create a corresponding question with:
        - THE SAME question type and format
        - THE SAME structural pattern
        - SIGNIFICANTLY MORE DETAILED and DESCRIPTIVE content
        - DIFFERENT numerical values, variables, names, or scenarios
        - THE SAME level of difficulty and complexity
        - RICH contextual information and real-world scenarios
    
    *   **Make Questions More Elaborate By:**
        - Adding relevant background context and real-world scenarios
        - Including descriptive details about situations, objects, or scenarios
        - Providing additional parameters and conditions that make the problem more realistic
        - Using compound sentences and detailed descriptions rather than bare statements
        - Incorporating multiple entities, characters, or variables with specific attributes
        - Adding temporal, spatial, or conditional clauses to enrich the narrative
        - Including units, measurements, and specific quantities with context
        - Weaving in domain-specific terminology and professional language
        
    *   **Examples of Elaboration:**
        - TEMPLATE: "A train travels 120 km in 2 hours. Find its speed."
        - ELABORATE: "A long-distance express train departing from Mumbai Central station travels through the Western Ghats, covering a total distance of 120 kilometers in exactly 2 hours while maintaining a constant velocity throughout its journey. Calculate the average speed of the train in kilometers per hour."
        
        - TEMPLATE: "Find the area of a circle with radius 7 cm."
        - ELABORATE: "A circular garden in a residential park has been designed with a radius measuring 7 centimeters on the architectural blueprint (scale 1:100). The landscaping team needs to calculate the actual area of this circular garden plot to estimate the amount of grass seed required for complete coverage. Determine the area of the circle using π = 22/7."

3.  **Enhanced Content Requirements:**
    *   Every question must tell a story or present a realistic scenario
    *   Include specific names, places, organizations, or objects when relevant
    *   Add descriptive adjectives and contextual phrases
    *   Incorporate multiple clauses and detailed conditions
    *   Ensure elaboration adds value without compromising clarity
    *   Maintain mathematical/logical precision despite added descriptiveness

4.  **Reference PYQ Usage:**
    *   Use PYQ materials to understand typical value ranges, realistic scenarios, and domain-specific terminology
    *   Extract contextual patterns and elaboration styles from PYQs
    *   Ensure your modified values are contextually appropriate and realistic

5.  **User Instructions:**
    *   Follow any specific user requirements for modifications or customizations
    *   If user instructions conflict with template structure, prioritize maintaining template structure while maximizing elaboration
...
7.  **Format as a Single, Complete JSON Object:**
    *   The ENTIRE output MUST be a single JSON object. Do not wrap it in markdown or add any text outside the JSON structure.
    *   The JSON object must strictly adhere to the following schema:
    \`\`\`json
    {
...
8.  **Diagram Generation (SVG):**
    *   If the template includes diagrams, create similar diagrams with modified parameters
    *   All diagrams must be drawn using **inline SVG** string elements embedded directly in the \`svg\` fields of the JSON
    *   Add labels, annotations, and detailed markings to make diagrams more descriptive
...
9.  **Content Rules:**
    ...
    *   **Solution Structure Requirements:**
        *   The \`steps\` field must contain brief, ordered reasoning points
        *   Each step should be concise and focus on one key logical progression
        *   Keep steps short and direct - avoid verbose explanations in solutions
        *   Final step should state the correct answer choice
    *   **Elaboration Balance:**
        *   Questions should be elaborate and descriptive
        *   Solutions should remain concise and step-focused
        *   Options should be detailed enough to be distinct and clear

10. **Quality Standards:**
    *   All modified values must be mathematically/logically consistent
    *   Options should remain plausible and follow the same pattern as template, but with more descriptive text
    *   The correct answer should be determinable using the same reasoning approach as the template
    *   Maintain professional language and formatting throughout
    *   Ensure elaboration enhances comprehension rather than causing confusion
    *   Questions should read naturally and professionally, not artificially inflated`;
...
class ContinuationHandler {
    ...
    detectIncomplete(jsonText) {
        try {
                    ...
                    if (section.questionSets && Array.isArray(section.questionSets)) {
                        ...
                        if (lastSet && lastSet.questions && Array.isArray(lastSet.questions)) {
                            ...
                            if (!lastQuestion.solution || 
                                ...
                                return { incomplete: true, parsed, reason: 'incomplete_question' };
                            }
                        }
                    }
                }
            ...
            return { incomplete: false, parsed };
        } catch (error) {
    ...
    /**
     * Repairs incomplete JSON by closing structures
     */
    repairJson(jsonText) {
    ...
    mergeJsonResponses(baseJson, continuationJson) {
        ...
        // Merge sections - the most critical part
                    ...
                    if (newSection.questionSets && Array.isArray(newSection.questionSets)) {
                        ...
                        newSection.questionSets.forEach((newSet, setIndex) => {
                            if (setIndex < existingSection.questionSets.length) {
                                // Question set exists - merge questions
                                ...
                                if (newSet.questions && Array.isArray(newSet.questions)) {
                                    ...
                                    const existingQNumbers = new Set(
                                        existingSet.questions.map(q => q.questionNumber)
                                    );
                                    
                                    newSet.questions.forEach(newQ => {
                                        if (!existingQNumbers.has(newQ.questionNumber)) {
        ...
        // Update exam details if present in continuation
        if (continuationJson.examTitle) merged.examTitle = continuationJson.examTitle;
        if (continuationJson.examDetails) merged.examDetails = continuationJson.examDetails;
        if (continuationJson.instructions) merged.instructions = continuationJson.instructions;
        
        return merged;
    }

    /**
     * Creates a continuation prompt with context
     */
    createContinuationPrompt(accumulatedJson, attemptNumber) {
        ...
        return `CONTINUATION REQUEST (Attempt ${attemptNumber})

You were generating a mock test JSON but reached the token limit. The last complete question was: ${lastQuestionNum}

CONTINUE from where you left off. Generate the remaining questions following the same template structure.

CRITICAL INSTRUCTIONS:
1. Start with a fresh, complete JSON object
2. Include ALL sections from the beginning
3. Include all previously generated questions
4. Continue adding the remaining questions
5. Ensure the JSON is valid and properly closed
6. Do NOT use "continuation_needed": true in the final output
7. Output ONLY the complete JSON object

Generate the complete final JSON now:`;
    }

    /**
     * Validates that merged JSON has all required questions
     */
    validateCompleteness(jsonData, expectedQuestions) {
        if (!expectedQuestions) return true; // Can't validate without expected count
        ...
        return totalQuestions >= expectedQuestions * 0.9; // Allow 10% tolerance
    }
}
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            ...
            if (incompletenessCheck.incomplete) {
                console.log(`Detected incomplete JSON (${incompletenessCheck.reason}). Attempting repair...`);
                ...
                try {
                    ...
                    // If we have accumulated JSON and repair failed, try continuation
                    if (continuationAttempts < maxContinuations) {
                        if (accumulatedJson) {
                            console.log(`Will continue with existing accumulated data...`);
                        } else if (incompletenessCheck.parsed) {
                            console.log(`Saving partial data for continuation...`);
                            accumulatedJson = incompletenessCheck.parsed;
                        }
                        continuationAttempts++;
                        attempt--; // Don't count as retry
                        continue;
                    } else {
                        throw new Error(`Max continuations reached. Last error: ${repairError.message}`);
                    }
                }
            } else {
            ...
            if (accumulatedJson && continuationAttempts > 0) {
                console.log(`Merging continuation data...`);
                jsonData = continuationHandler.mergeJsonResponses(accumulatedJson, jsonData);
            }

            // Check if continuation is still needed
