...
3.  **Data Interpretation Requirements:**
    ...
    *	For Data Sufficiency questions string, IT MUST HAVE STATEMENTS LIKE STATEMENT 1 AND STATEMENT 2 BEFORE QUESTIONS
...
5.  **Format as a Single, Complete JSON Object:**
    ...
    *   The JSON object must strictly adhere to the following schema:
    \`\`\`json
    {
      "examTitle": "String", // e.g., "Data Interpretation Mock Test for SRCC GBO"
      "examDetails": {
              ...
              "questions": [
                {
                  ...
                  "questionText": "String", // The question text, Should have statements like statement 1 and statement 2 for data sufficiency questions
                  "options": [
                    {
...
function createChartSlide(pptx, questionSet, bgImagePath) {
    ...
    if (questionSet.chartData) {
        ...
        // Add the chart with proper sizing to fit within slide bounds
        const base64Svg = svgToBase64(questionSet.chartData.svg);
        if (base64Svg) {
            slide.addImage({
                data: base64Svg,
                x: 0.5, y: 2.0, w: 5, h: 3.0,
                sizing: { type: 'contain', w: 9, h: 5.0 }
            });
        }
    }
}
...
function createChartSlide(pptx, questionSet, bgImagePath) {
    ...
    if (questionSet.chartData) {
        ...
        // Add the chart (scaled down to 50%)
        const base64Svg = svgToBase64(questionSet.chartData.svg);
        if (base64Svg) {
            // original size
            const origW = 5.0;
            ...
            // slide dimensions (default pptxgenjs = 10" x 5.625")
            const slideW = 10;
            const slideH = 5.625;

            // center the image horizontally & place below title
            const posX = (slideW - newW) / 2;
            const posY = 2.0 + (origH - newH) / 2; // shift a little to keep aligned
...
async function generatePptFromJson(jsonData, outputPath, backgroundPath) {
	...
	const outDir = path.dirname(outputPath);
        ...
        console.log(`Converting to .ppt: ${command}`);
        ...
        //console.log(`Original .pptx deleted: ${path.basename(outputPath)}`);
