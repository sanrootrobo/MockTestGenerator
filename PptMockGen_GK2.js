...
// --- UPDATED SYSTEM PROMPT FOR X-GMT ---
const systemPrompt = `You are an expert exam designer and question creator specializing in X-GMT (Xavier General Management Aptitude Test) entrance exams. Your primary task is to generate a BRAND NEW, high-quality mock test and output it as a single, complete, and valid JSON object.

Follow these rules with absolute precision:

1. **Analyze Reference Materials:**
   * Carefully study all the provided "REFERENCE PYQ PDF" documents to understand X-GMT question styles, common topics, difficulty level, and typical phrasing patterns.
   * Examine the "REFERENCE Mock Test PDF" documents to understand their structure and the tone of their instructions.
   * **CRITICAL**: Study the provided "REFERENCE Mock Dir" materials thoroughly. This directory contains current affairs content that MUST be used exclusively as the source for all General Awareness questions. All GA questions must be derived from the information contained within these reference mock materials.

2. **Generate Original Content:**
   * You MUST NOT copy any questions or passages directly from the reference materials or current affairs files.
   * All questions, options, and solutions you generate must be entirely new and unique.
   * For General Awareness questions: Base them on events, policies, appointments, awards, economic developments, and international affairs mentioned in the "REFERENCE Mock Dir" materials, but frame them as original analytical questions.
   * For Emotional Quotient questions: Create realistic workplace scenarios testing integrity, leadership, team management, and professional judgment.

3. **Content Requirements for X-GMT Format:**
   * **Section 1: General Awareness (20 questions)** - Must be based exclusively on REFERENCE Mock Dir materials
     - Current Affairs & International Developments (6-7 questions)
     - Government Policies & Political Developments (4-5 questions) 
     - Business & Economic Affairs (4-5 questions)
     - Awards, Personalities & Achievements (4-5 questions)
   * **Section 2: Emotional Quotient & Situational Judgment (20 questions)**
     - Workplace Ethics & Integrity Scenarios (5-6 questions)
     - Team Management & Leadership (5-6 questions) 
     - Professional Relationships & Conflict Resolution (4-5 questions)
     - Self-Assessment & Personality Traits (4-5 questions)

4. **Question Quality Standards:**
   * **Hard Difficulty Level**: Questions should require deep analysis, inference, and critical thinking
   * **General Awareness**: Focus on "why" behind events, strategic implications, cause-effect relationships, and connections between multiple pieces of information from REFERENCE Mock Dir materials
   * **Emotional Quotient**: Present realistic workplace dilemmas with ethically complex situations requiring professional judgment
   * All options should be plausible but clearly distinguishable for analytical minds

5. **Process User Instructions:**
   ...
   * Maintain X-GMT authenticity while incorporating user specifications.
...
6. **Format as a Single, Complete JSON Object:**
   ...
   * The JSON object must strictly adhere to the following schema:
   \`\`\`json
   {
     "examTitle": "String", // e.g., "X-GMT 2025 General Awareness & Emotional Quotient Mock Test"
     "examDetails": {
       "totalQuestions": Number, // Always 40 for X-GMT
       "timeAllotted": "String", // e.g., "120 Minutes" 
       "maxMarks": Number,
       "sections": 2,
       "negativeMarking": "String" // e.g., "Yes, 1/4th mark deducted for wrong answers"
     },
     "instructions": {
       "title": "String", // e.g., "General Instructions"
       "points": ["String", "String", ...] // An array of instruction points specific to X-GMT format
     },
     "sections": [
       {
         "sectionTitle": "String", // e.g., "Section 1: General Awareness"
         "sectionDetails": {
           "questionsCount": Number,
           "timeLimit": "String", // If sectional timing applies
           "description": "String" // Brief description of section focus
         },
         "questionSets": [
           {
             "type": "group | single", // "group" if there are shared directions, "single" otherwise
             "directions": { // Optional: Only include if type is "group"
               "title": "String", // e.g., "Questions 1-7: Current Affairs & International Developments"
               "text": "String" // The directions text. Can include HTML like <br> or <ul>.
             },
             "questions": [
               {
                 "questionNumber": "String", // e.g., "Q1", "1"
                 ...
                 "solution": {
                   "answer": "String", // e.g., "Option (B)"
                   "explanation": "String", // Detailed explanation following the specified answer key format
                   "competencyTested": "String", // For EQ questions: primary competency being evaluated
                   "sourceDocument": "String" // For GA questions: which reference material from Mock Dir
                 }
               }
             ]
           }
...
7. **Answer Key Requirements:**
   * **For General Awareness Questions:**
     - Provide background context from REFERENCE Mock Dir materials
     - Give direct answer analysis with specific references to source material
     - Explain why other options are incorrect with document-based reasoning
     - Connect to broader significance within the context of current affairs from REFERENCE Mock Dir
   
   * **For Emotional Quotient Questions:**
     - Identify primary and secondary competencies tested
     - Evaluate pros/cons of each option with behavioral implications
     - Provide strategic rationale for the best choice
     - Explain why the least effective choice is detrimental

8. **Content Authenticity:**
   * Ensure every question has a corresponding solution with clear answer and explanation.
   * The \`questionNumber\` for each question must be unique and sequential.
   * Generate content that reflects contemporary context and current business/management practices.
   * Maintain professional tone appropriate for MBA-level candidates.
   * Questions should be solvable and free of contradictions.

9. **Critical Restrictions:**
   * NO direct copying from reference materials
   * General Awareness questions MUST derive from provided REFERENCE Mock Dir materials only
   * Maintain X-GMT exam authenticity and professional standards throughout`;
...
<body>
    <div class="test-header">
        ...
        <div class="test-info">
            ...
            <div class="test-info-item">
                <div class="test-info-label">Sections</div>
                <div class="test-info-value">${jsonData.examDetails.sections}</div>
            </div>
        </div>
        ${jsonData.examDetails.negativeMarking ? `
            <div class="negative-marking">
                Negative Marking: ${jsonData.examDetails.negativeMarking}
            </div>
        ` : ''}
    </div>
        ...
        <div class="section-header">
            ...
            ${section.sectionDetails ? `
                <div class="section-details">
                    ${section.sectionDetails.description || ''} 
                    (${section.sectionDetails.questionsCount} questions${section.sectionDetails.timeLimit ? ` - ${section.sectionDetails.timeLimit}` : ''})
                </div>
            ` : ''}
        </div>
            ...
            section.questionSets.map(questionSet => 
                questionSet.questions.map(question => `
                    <div class="answer-item">
                        ...
                        ${question.solution.competencyTested || question.solution.sourceDocument ? `
                            <div class="answer-metadata">
                                ${question.solution.competencyTested ? `Competency Tested: ${question.solution.competencyTested}` : ''}
                                ${question.solution.competencyTested && question.solution.sourceDocument ? ' | ' : ''}
                                ${question.solution.sourceDocument ? `Source: ${question.solution.sourceDocument}` : ''}
                            </div>
                        ` : ''}
                    </div>
                `).join('')
...
function createTitleSlide(pptx, data, bgImagePath) {
    ...
    const detailsText = `Questions: ${details.totalQuestions}  |  Time: ${details.timeAllotted}  |  Marks: ${details.maxMarks}  |  Sections: ${details.sections}`;
    slide.addText(detailsText, {
        x: 0.5, y: 3.0, w: '90%', h: 0.5, fontSize: 18, color: '333333', align: 'center',
    });
    
    if (details.negativeMarking) {
        slide.addText(`Negative Marking: ${details.negativeMarking}`, {
            x: 0.5, y: 3.7, w: '90%', h: 0.3, fontSize: 16, color: '666666', align: 'center',
        });
    }
}
...
function createSectionSlide(pptx, section, bgImagePath) {
    const slide = addSlideWithBackground(pptx, bgImagePath);
    slide.addText(section.sectionTitle, { 
        x: 0, y: '35%', w: '100%', align: 'center', 
        fontSize: 36, color: '003B75', bold: true 
    });
    
    if (section.sectionDetails && section.sectionDetails.description) {
        slide.addText(section.sectionDetails.description, {
            x: 0.5, y: '55%', w: '90%', align: 'center',
...
function createQuestionSlide(pptx, question, directions, bgImagePath) {
    ...
    const questionTextHeight = question.questionText.length > 200 ? 1.8 : 1.2;
    ...
    question.options.forEach(opt => {
        ...
        slide.addText(optionText, { 
            x: 0.75, y: currentY, w: '85%', h: 0.35, fontSize: 14 
        });
        currentY += 0.4;
    });
}
...
function createAnswerSlide(pptx, question, bgImagePath) {
    ...
    slide.addText(explanationText, {
        x: 0.5, y: 1.6, w: '90%', h: 3.5, fontSize: 12,
    });
    
    if (question.solution.competencyTested || question.solution.sourceDocument) {
        let metadataText = '';
        if (question.solution.competencyTested) {
            metadataText += `Competency: ${question.solution.competencyTested}`;
        }
        if (question.solution.sourceDocument) {
            metadataText += `${metadataText ? ' | ' : ''}Source: ${question.solution.sourceDocument}`;
        }
...
async function generatePptFromJson(jsonData, outputPath, backgroundPath) {
    try {
        ...
        jsonData.sections.forEach(section => {
            ...
            createSectionSlide(pptx, section, backgroundPath);
