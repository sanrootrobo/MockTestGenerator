              ...
              "questions": [
                {
                  ...
                  "solution": {
                    "answer": "String", // e.g., "Option (D) – R"
                    "steps": [
                      "String", // Step 1: brief reasoning point
                      "String", // Step 2: brief reasoning point
                      "String"  // Step 3: brief reasoning point, etc.
...
function convertHtmlToPptxRichText(html) {
    ...
    // Handle complex formatting including tables and lists
...
// 1. IMPROVED SVG HANDLING - Better size constraints and responsive design
function svgToBase64(svgContent) {
    ...
    // Ensure SVG has proper dimensions for better rendering
    if (!svgString.includes('viewBox=')) {
        // Add default viewBox if missing
        svgString = svgString.replace('<svg', '<svg viewBox="0 0 400 300"');
    }

    // Ensure width and height are set for consistency
...
function createQuestionSlide(pptx, question, directions, bgImagePath) {
    ...
    // Directions with improved formatting
    if (directions) {
    ...
    // Question text with dynamic height calculation
    ...
    // SVG/Chart handling with better positioning and sizing
    if (question.svg) {
        const base64Svg = svgToBase64(question.svg);
        if (base64Svg) {
            // Determine optimal image size based on available space
            ...
            const imageWidth = imageHeight * 1.33; // 4:3 aspect ratio
    ...
    question.options.forEach(opt => {
        if (optionsPerRow === 2 && optionCount % 2 === 1) {
            optionX = 5.2; // Second column
        } else {
            optionX = 0.5; // First column
        }
...
/*function createAnswerSlide(pptx, question, bgImagePath) {
    ...
    // Explanation
    const explanationText = question.solution.explanation
        .replace(/<[^>]*>/g, ' ')
        .replace(/\s+/g, ' ')
        .trim();
