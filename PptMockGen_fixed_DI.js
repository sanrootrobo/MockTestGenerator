...
// Define the strict JSON schema using Gemini's Type system
const MOCK_TEST_SCHEMA = {
    type: Type.OBJECT,
    properties: {
        examTitle: {
            type: Type.STRING,
        ...
        examDetails: {
            ...
            properties: {
                ...
                timeAllotted: {
                    type: Type.STRING,
                    description: "Time duration for the exam (e.g., '3 hours')"
                },
                maxMarks: {
                    type: Type.INTEGER,
            ...
            required: ["totalQuestions", "timeAllotted", "maxMarks"],
            propertyOrdering: ["totalQuestions", "timeAllotted", "maxMarks"]
        },
        instructions: {
            type: Type.OBJECT,
            properties: {
                title: {
                    type: Type.STRING,
                    description: "Title of the instructions section"
                },
                points: {
                    type: Type.ARRAY,
                    items: {
                    ...
                    description: "Array of instruction points"
                }
            },
        ...
        sections: {
            ...
            items: {
                ...
                properties: {
                    sectionTitle: {
                        type: Type.STRING,
                        description: "Title of the exam section"
                    },
                    questionSets: {
                        type: Type.ARRAY,
                        items: {
                            ...
                            properties: {
                                ...
                                directions: {
                                    ...
                                    properties: {
                                        title: {
                                            type: Type.STRING,
                                            description: "Title of the directions"
                                        },
                                        text: {
                                            type: Type.STRING,
                                            description: "Directions text"
                                        }
                                    },
                                    required: ["title", "text"],
                                    ...
                                    items: {
                                        ...
                                        properties: {
                                            ...
                                            svg: {
                                                type: Type.STRING,
                                                nullable: true,
                                                description: "SVG diagram for the question if needed"
                                            },
                                            options: {
                                                type: Type.ARRAY,
                                                items: {
                                                    ...
                                                    properties: {
                                                        ...
                                                        svg: {
                                                            type: Type.STRING,
                                                            nullable: true,
                                                            description: "SVG diagram for the option if needed"
                                                        }
                                                    },
                                                    required: ["label", "text"],
                                                    propertyOrdering: ["label", "text", "svg"]
                                                },
                                                minItems: 2,
                                                maxItems: 6
                                            },
                                            solution: {
                                                ...
                                                properties: {
                                                    answer: {
                                                        type: Type.STRING,
                                                        description: "Correct answer (option label)"
                                                    },
                                                    explanation: {
                                                        type: Type.STRING,
                                                    ...
                                                    svg: {
                                                        type: Type.STRING,
                                                        nullable: true,
                                                        description: "SVG diagram for the solution if needed"
                                                    }
                                                },
                                                required: ["answer", "explanation"],
                                                propertyOrdering: ["answer", "explanation", "svg"]
                                            }
                                        },
                                        required: ["questionNumber", "questionText", "options", "solution"],
                                        propertyOrdering: ["questionNumber", "questionText", "svg", "options", "solution"]
                                    }
                                }
                            },
                            required: ["type", "questions"],
                            propertyOrdering: ["type", "directions", "questions"]
                        }
                    }
                },
                required: ["sectionTitle", "questionSets"],
                propertyOrdering: ["sectionTitle", "questionSets"]
            }
        }
    },
    required: ["examTitle", "examDetails", "instructions", "sections"],
    propertyOrdering: ["examTitle", "examDetails", "instructions", "sections"]
};

const systemPrompt = `You are an expert exam designer and question creator specializing in competitive entrance exams. Your primary task is to generate a BRAND NEW, high-quality mock test.
...
4. **Content Requirements:**
   ...
   * Keep explanations comprehensive but under 250 characters for optimal processing.
...
5. **Diagram Generation (SVG):**
   ...
   * All diagrams must be drawn using inline SVG string elements.
...
7. Enhanced Answer Format & Solution Style
- **Multiple Choice:** Four options (A, B, C, D) for each question
- **Solutions:** Provide detailed, step-by-step analytical breakdown
    - **Data Identification**: Highlight relevant data points
    - **Method Selection**: State the approach/formula used
    - **Calculation Steps**: Show all intermediate calculations
    - **Verification**: Cross-check with given options
    - **Alternative Methods**: Mention quicker approaches where applicable



Generate the complete mock test following the specified JSON schema structure exactly.`;
...
// --- JSON TO PPTX CONVERSION (unchanged) ---
...
// --- HELPER FUNCTIONS (updated for strict JSON) ---
...
function createGenerationConfig(options, model) {
    const config = {
        ...
        responseSchema: MOCK_TEST_SCHEMA
    };
...
// --- MAIN MOCK GENERATION FUNCTION (updated for strict JSON schema) ---
async function generateSingleMock(contents, outputPath, mockNumber, totalMocks, options) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            ...
            console.log(`📋 Using strict JSON schema validation`);
            
            // Create generation config with strict schema
            ...
            // Parse and validate JSON - handle both structured and markdown-wrapped responses
            let jsonData;
            try {
                ...
                if (cleanJson.startsWith('```json')) {
                    console.log(`⚠️  Warning: API returned markdown-wrapped JSON despite schema - cleaning...`);
                    cleanJson = cleanJson.replace(/^```json\s*/, '').replace(/\s*```$/, '');
                } else if (cleanJson.startsWith('```')) {
                    console.log(`⚠️  Warning: API returned markdown-wrapped content despite schema - cleaning...`);
                    cleanJson = cleanJson.replace(/^```\s*/, '').replace(/\s*```$/, '');
                } else {
                    console.log(`✅ API returned clean JSON as expected`);
                }
                
                jsonData = JSON.parse(cleanJson);
                console.log(`✅ JSON parsing successful for mock ${mockNumber}`);
            } catch (parseError) {
                // Log the problematic content for debugging
                console.error(`❌ JSON parsing failed for mock ${mockNumber}`);
                console.error(`First 200 chars of response: ${generatedJson.substring(0, 200)}...`);
                throw new Error(`JSON parsing error: ${parseError.message}`);
            }

            // Additional validation for business logic
            if (!jsonData.examTitle || !jsonData.examDetails || !jsonData.sections) {
                throw new Error("Invalid JSON structure - missing required top-level fields");
            }
            
            if (jsonData.sections.length === 0) {
                throw new Error("Invalid JSON structure - no sections provided");
            }
            
            // Validate that all questions have unique numbers
                ...
                for (const questionSet of section.questionSets) {
                    for (const question of questionSet.questions) {
                        if (questionNumbers.has(question.questionNumber)) {
                            throw new Error(`Duplicate question number found: ${question.questionNumber}`);
                        }
            ...
            console.log(`✅ Schema validation passed: ${totalQuestions} questions generated`);
            ...
            if (options.saveJson) {
                ...
                try {
                    ...
                    console.log(`[DEBUG] Structured JSON for mock ${mockNumber} saved to: ${debugJsonPath}`);
                } catch(e) {
            ...
            console.log(`🔄 Converting structured JSON to HTML for mock ${mockNumber}...`);
            ...
            console.log(`📄 Structured JSON content length: ${generatedJson.length} characters`);
            ...
            if (isSchemaError) {
                console.error(`❌ Schema validation error for mock ${mockNumber}: ${error.message}`);
                // Don't retry schema errors - they indicate a fundamental issue
                break;
            }
...
async function main() {
    ...
    try {
        ...
        // 6. Generate mock tests with strict JSON schema
        console.log(`\n🚀 Starting generation of ${numberOfMocks} mock test(s) with strict JSON schema...`);
        let outputFormats = ["Structured JSON", "PDF"];
        ...
        console.log(`🔒 Schema Compliance: 100% (guaranteed by Gemini structured output)`);
        ...
        console.log(`\n🎉 Successfully generated ${successful.length} structured mock test(s)!`);
