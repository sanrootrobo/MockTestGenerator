...
function createChartSlide(pptx, questionSet, bgImagePath) {
    ...
    if (questionSet.directions) {
        ...
        const cleanDirections = questionSet.directions.text
            .replace(/<[^>]*>/g, ' ')
            .replace(/\s+/g, ' ')
            .trim();
        
        slide.addText(cleanDirections, {
            x: 0.3, y: 0.7, w: '90%', h: 0.8,
    ...
    if (questionSet.chartData) {
        const titleY = questionSet.directions ? 1.6 : 0.5;
        
        slide.addText(questionSet.chartData.title, {
            x: 0.3, y: titleY, w: '90%', h: 0.6,
            fontSize: 20, fontFace: 'Calibri',
            bold: true, color: '2B6CB0', align: 'center'
        });
        
        slide.addText(questionSet.chartData.description, {
            x: 0.3, y: titleY + 0.7, w: '90%', h: 0.4,
            fontSize: 12, fontFace: 'Calibri',
            italic: true, color: '4A5568', align: 'center'
        });
        
        // Add the chart - taking up most of the slide space
        const base64Svg = svgToBase64(questionSet.chartData.svg);
        if (base64Svg) {
            const chartY = titleY + 1.2;
            const chartHeight = 7.5 - chartY - 0.3; // Use remaining space
            const chartWidth = chartHeight * 1.2; // Maintain good aspect ratio
            const chartX = (10 - chartWidth) / 2; // Center horizontally
...
function createQuestionSlideForDI(pptx, questions, questionSetIndex, bgImagePath) {
    const slide = addSlideWithBackground(pptx, bgImagePath);
    
    // Title indicating this contains questions for a specific chart
    slide.addText(`Questions ${questions[0].questionNumber}-${questions[questions.length-1].questionNumber}`, {
        x: 0.3, y: 0.2, w: '90%', h: 0.4,
        fontSize: 16, fontFace: 'Calibri',
        bold: true, color: '1A365D', align: 'center'
    });
    
    let currentY = 0.8;
    
    // Add all questions for this chart
    questions.forEach((question, index) => {
        // Question text
        const questionHeight = question.questionText.length > 150 ? 0.8 : 0.6;
        
        slide.addText(`${question.questionNumber}. ${question.questionText}`, {
            x: 0.3, y: currentY, w: '90%', h: questionHeight,
            fontSize: 11, fontFace: 'Calibri',
            bold: false, wrap: true
        });
        currentY += questionHeight + 0.1;
        ...
        question.options.forEach(opt => {
            const optionX = (optionCount % optionsPerRow === 0) ? 0.5 : 5.2;
            ...
            if (optionCount % optionsPerRow === 1 || optionCount === question.options.length - 1) {
                currentY += 0.35;
            }
            optionCount++;
        });
        
        currentY += 0.2; // Space between questions
        
        // Check if we need to break to next slide
        if (currentY > 6.5 && index < questions.length - 1) {
            // This question set is too long, should be split
            // For now, we'll continue on the same slide but with smaller spacing
            currentY -= 0.1;
        }
    });
}

function createAnswerSlideForDI(pptx, questions, bgImagePath) {
    const slide = addSlideWithBackground(pptx, bgImagePath);

    // Title
    slide.addText(`Solutions: Questions ${questions[0].questionNumber}-${questions[questions.length-1].questionNumber}`, {
        x: 0.3, y: 0.2, w: '90%', h: 0.4,
        fontSize: 14, fontFace: 'Calibri',
        bold: true, color: '1A365D', align: 'center'
    });
    ...
    questions.forEach((question, index) => {
        // Answer
        slide.addText(`${question.questionNumber}: ${question.solution.answer}`, {
        ...
        if (currentY > 6.5 && index < questions.length - 1) {
            currentY -= 0.3; // Reduce spacing to fit more
        }
    });
}
...
async function generatePptFromJson(jsonData, outputPath, backgroundPath) {
    try {
        ...
        // Process each question set (each with its own chart)
        jsonData.sections.forEach(section => {
            section.questionSets.forEach((questionSet, setIndex) => {
                ...
                // 2. Create a slide for all questions related to this chart
                console.log(`Creating questions slide for question set ${setIndex + 1}...`);
                createQuestionSlideForDI(pptx, questionSet.questions, setIndex, backgroundPath);
            });
        });
        ...
        // Create answer slides for each question set
        jsonData.sections.forEach(section => {
            section.questionSets.forEach((questionSet, setIndex) => {
                console.log(`Creating answer slide for question set ${setIndex + 1}...`);
                createAnswerSlideForDI(pptx, questionSet.questions, backgroundPath);
            });
        });
