...
async function generateSingleMock(contents, outputPath, mockNumber, totalMocks, options) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            ...
            const genAI = new GoogleGenAI(currentKeyInfo.key); // Fixed: removed object wrapper
            ...
            // Prepare request parameters for generateContentStream
            const requestParams = {
                contents: contents
            };
            ...
            console.log(`\n${'='.repeat(80)}`);
            console.log(`📋 STREAMING CONTENT FOR MOCK ${mockNumber}/${totalMocks}`);
            console.log(`${'='.repeat(80)}\n`);
            
            // Use the model's generateContentStream method directly
            const streamingResponse = await model.generateContentStream(requestParams);
            
            let generatedContent = '';
            let chunkCount = 0;
            
            // Process streaming chunks - the streamingResponse itself is the async iterator
            ...
            console.log(`📄 Total generated content length: ${generatedContent.length} characters`);
            console.log(`${'='.repeat(80)}\n`);
            ...
            // Get final response for metadata (if available)
            let finalResponse = null;
            try {
                // Try to get the response metadata if the streaming response has it
                if (streamingResponse.response) {
                    finalResponse = await streamingResponse.response;
                }
            } catch (e) {
                console.warn(`⚠️ Could not get response metadata: ${e.message}`);
            }
            ...
            // Parse and validate JSON (keep this for structure validation)
            let jsonData = null;
            try {
                ...
                try {
                    ...
                    console.log(`🔧 First cleaning attempt failed, trying aggressive cleaning...`);
                    cleanJson = aggressiveJsonCleaning(generatedContent.trim());
                    if (cleanJson.startsWith('```json')) {
                        cleanJson = cleanJson.replace(/^```json\s*/, '').replace(/\s*```$/, '');
                    } else if (cleanJson.startsWith('```')) {
                        cleanJson = cleanJson.replace(/^```\s*/, '').replace(/\s*```$/, '');
                ...
                console.log(`🔍 Error occurred around position ${parseError.message.match(/position (\d+)/)?.[1] || 'unknown'}`);
                
                // Try to show problematic area for debugging
                if (parseError.message.includes('position')) {
                    const position = parseInt(parseError.message.match(/position (\d+)/)?.[1] || '0');
                    const start = Math.max(0, position - 100);
                    const end = Math.min(generatedContent.length, position + 100);
                    const context = generatedContent.substring(start, end);
                    console.log(`📍 Context around error (chars ${start}-${end}):`);
                    console.log(`"${context}"`);
                }
            ...
            // Validate JSON structure if parsing succeeded
            if (jsonData && (!jsonData.examTitle || !jsonData.examDetails || !jsonData.sections)) {
                console.warn(`⚠️ Invalid JSON structure detected - missing required fields`);
            }
            ...
            // Save debug content file if requested
            if (options.saveJson) {
                const debugContentPath = generateDebugJsonFilename(options.output, mockNumber, totalMocks);
                try {
                    await fs.writeFile(debugContentPath, generatedContent);
                    console.log(`[DEBUG] Raw generated content for mock ${mockNumber} saved to: ${debugContentPath}`);
                } catch(e) {
                    console.error(`[DEBUG] Failed to save debug content file: ${e.message}`);
                }
            }

            // If JSON parsing succeeded, continue with file generation
            if (jsonData) {
                ...
                // Save raw content as text file if JSON parsing failed
                const textOutputPath = outputPath.replace('.pdf', '.txt');
                await fs.writeFile(textOutputPath, generatedContent);
                console.log(`📄 Raw content saved as text file: ${path.basename(textOutputPath)}`);
            }
