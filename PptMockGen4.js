...
// --- JSON TO HTML CONVERSION WITH FONT OPTIONS ---
function convertJsonToHtml(jsonData, fontOptions = {}) {
    const { fontName = 'Inter', fontSize = 14 } = fontOptions;
    
    // Calculate relative font sizes based on the base font size
    const baseFontSize = fontSize;
    const headerFontSize = Math.round(baseFontSize * 2.0); // 28px default -> 2x base
    const subHeaderFontSize = Math.round(baseFontSize * 1.57); // 22px default -> 1.57x base
    const sectionTitleFontSize = Math.round(baseFontSize * 1.29); // 18px default -> 1.29x base
    const questionNumberFontSize = Math.round(baseFontSize * 1.14); // 16px default -> 1.14x base
    const answerKeyFontSize = Math.round(baseFontSize * 1.07); // 15px default -> 1.07x base
    const optionFontSize = Math.round(baseFontSize * 0.93); // 13px default -> 0.93x base
    const instructionFontSize = baseFontSize; // Same as base
...
<head>
    ...
    <style>
        ...
        * {
        ...
        .test-info-value {
            font-weight: 600;
            font-size: ${Math.round(baseFontSize * 1.14)}px;
        }
        ...
        .question-number {
            font-weight: 600;
            color: #2b6cb0;
            font-size: ${questionNumberFontSize}px;
        }
        ...
        .answer-key {
            font-weight: 600;
            color: #22543d;
            font-size: ${answerKeyFontSize}px;
            margin-bottom: 8px;
        }
        ...
        @media print {
            body {
                font-size: ${Math.round(baseFontSize * 0.86)}px;
                line-height: 1.4;
                padding: 10px;
            }
            
            .test-header h1 {
                font-size: ${Math.round(headerFontSize * 0.86)}px;
            }
        ...
        @media screen and (max-width: 768px) {
            body {
                padding: 10px;
                font-size: ${Math.round(baseFontSize * 0.93)}px;
            }
...
// --- JSON TO PPTX CONVERSION WITH FONT OPTIONS ---
...
/**
 * --- NEW: Converts an SVG string to a Base64 encoded PNG using Puppeteer. ---
 * This provides higher compatibility for PowerPoint than SVG.
 * @param {string} svgContent The SVG string to convert.
 * @returns {Promise<string|null>} A Base64 encoded PNG data URL, or null on failure.
 */
async function svgToPngBase64(svgContent) {
    ...
    try {
        ...
        // Set a transparent background for the page
        await page.evaluate(() => {
            document.body.style.background = 'transparent';
        });

        // Set the page content with the SVG wrapped in basic HTML
...
<body>${svgContent}</body>
        ...
        // Take a screenshot of just the SVG element with a transparent background
...
function createTitleSlide(pptx, data, bgImagePath, fontOptions = {}) {
    ...
    slide.addText(detailsText, {
        ...
        fontSize: Math.round(fontSize * 1.43), // 20px equivalent
...
function createInstructionsSlide(pptx, data, bgImagePath, fontOptions = {}) {
    ...
    slide.addText(data.instructions.title, { 
        x: 0.5, y: 0.5, w: '90%', 
        fontSize: Math.round(fontSize * 2.29), // 32px equivalent
    ...
    const instructionPoints = data.instructions.points.map(point => ({ 
        text: point, 
        options: { 
            fontSize: Math.round(fontSize * 1.29), // 18px equivalent
            bullet: true, 
            paraSpcAfter: 10,
            fontFace: fontName
        } 
...
// --- MODIFIED: Function is now async to handle image conversion ---
async function createQuestionSlide(pptx, question, directions, bgImagePath, fontOptions = {}) {
    ...
    slide.addText(`Question ${question.questionNumber}`, { 
        x: 0.5, y: 0.4, w: '90%', 
        fontSize: Math.round(fontSize * 1.71), // 24px equivalent
    ...
    if (directions) {
        ...
        slide.addText(`Directions: ${cleanDirections}`, {
            ...
            fontSize: Math.round(fontSize * 0.86), // 12px equivalent
    ...
    slide.addText(convertHtmlToPptxRichText(question.questionText), {
        ...
        fontSize: Math.round(fontSize * 1.14), // 16px equivalent
        fontFace: fontName
    });
    currentY += questionTextHeight + 0.2;

    if (question.svg) {
        const pngBase64 = await svgToPngBase64(question.svg);
        if (pngBase64) {
    ...
    // --- MODIFIED: Use for...of loop to allow await inside ---
    for (const opt of question.options) {
        ...
        if (opt.svg) {
            ...
            const pngBase64 = await svgToPngBase64(opt.svg);
            if (pngBase64) slide.addImage({ data: pngBase64, x: 1.25, y: currentY - 0.25, w: 1, h: 1 });
            currentY += 1.2;
        } else {
...
async function createAnswerSlide(pptx, question, bgImagePath, fontOptions = {}) {
    ...
    // --- MODIFIED: Await the conversion and store in a variable ---
    const pngBase64 = await svgToPngBase64(question.solution.svg);
...
// --- MODIFIED: Loops updated to handle async slide creation ---
async function generatePptFromJson(jsonData, outputPath, backgroundPath, fontOptions = {}) {
    try {
        ...
        // --- MODIFIED: Use for...of loop to allow await ---
        ...
        answerTitleSlide.addText('Answers & Solutions', { 
            ...
            fontSize: Math.round((fontOptions.fontSize || 14) * 3.14), // 44px equivalent
...
function validateFontName(fontName) {
    ...
    // List of commonly supported fonts
    const supportedFonts = [
        'Inter', 'Calibre', 'Arial', 'Helvetica', 'Times New Roman', 'Georgia', 'Verdana',
        'Tahoma', 'Trebuchet MS', 'Comic Sans MS', 'Impact', 'Lucida Console',
        'Courier New', 'Palatino', 'Garamond', 'Bookman', 'Avant Garde',
        'Roboto', 'Open Sans', 'Lato', 'Montserrat', 'Source Sans Pro',
        'Poppins', 'Nunito', 'Raleway', 'Ubuntu', 'Merriweather'
    ];
    
    // Check if font exists in supported list (case insensitive)
    const normalizedInput = fontName.trim();
    const matchedFont = supportedFonts.find(font => 
        font.toLowerCase() === normalizedInput.toLowerCase()
    );
    
    if (matchedFont) {
        console.log(`✅ Using font: ${matchedFont}`);
        return matchedFont;
    } else {
        console.warn(`⚠️  Font '${fontName}' not in supported list. Using default: Inter`);
        console.log(`📝 Supported fonts: ${supportedFonts.join(', ')}`);
        return 'Inter';
    }
}
...
function validateFontSize(fontSize) {
    ...
    if (isNaN(size)) {
        console.warn(`⚠️  Invalid font size '${fontSize}'. Using default: 14px`);
        return 14;
    }
    
    if (size < 8) {
        console.warn(`⚠️  Font size ${size}px is too small. Using minimum: 8px`);
        return 8;
    }
    
    if (size > 72) {
        console.warn(`⚠️  Font size ${size}px is too large. Using maximum: 72px`);
        return 72;
    }
    
    console.log(`✅ Using font size: ${size}px`);
    return size;
}
...
async function generateSingleMock(contents, outputPath, mockNumber, totalMocks, options) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            ...
            // Convert JSON to HTML with font options
            ...
            console.log(`🎨 Font settings: ${fontOptions.fontName} (${fontOptions.fontSize}px)`);
...
async function main() {
    program
        ...
        .option("--fontname <name>", "Font name for questions, options, and answers (default: Inter)")
        .option("--fontsize <size>", "Font size in pixels (8-72, default: 14)", parseInt)
        .parse(process.argv);
    ...
    try {
        ...
        console.log(`🎨 Typography: ${validatedFontName} font at ${validatedFontSize}px`);
        if (options.saveJson) {
        ...
        if (successful.length > 0) {
            ...
            console.log(`  Font: ${validatedFontName}`);
            console.log(`  Size: ${validatedFontSize}px`);
        }
