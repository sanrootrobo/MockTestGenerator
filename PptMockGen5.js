...
function convertJsonToHtml(jsonData, fontOptions = {}) {
    // --- MODIFIED: Default font is Arial, default size is 11pt ---
    ...
    // --- MODIFIED: All font sizes now use 'pt' unit ---
...
function createGenerationConfig(options, model) {
    ...
    if (options.temperature && options.temperature !== 0.4) {
...
// --- MODIFIED: Allows any font name, defaults to Arial for broad compatibility ---
function validateFontName(fontName) {
    ...
    // Note: The user is responsible for ensuring this font is available 
    // where the PDF/PPTX is generated and viewed.
    return sanitizedFont;
}

// --- MODIFIED: Validates font size in points (pt) ---
...
async function generateSingleMock(contents, outputPath, mockNumber, totalMocks, options) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            ...
            console.log(`🎨 Font settings: ${fontOptions.fontName} (${fontOptions.fontSize}pt)`);
...
async function main() {
    program
        ...
        // --- MODIFIED: Font options updated for points and flexibility ---
    ...
    try {
        ...
        console.log(`🎨 Typography: ${validatedFontName} font at ${validatedFontSize}pt`);
        if (options.saveJson) {
        ...
        if (successful.length > 0) {
            ...
            console.log(`  Size: ${validatedFontSize}pt`);
        }
