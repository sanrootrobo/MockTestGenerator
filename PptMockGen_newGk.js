...
1.  **Analyze Reference Materials:**
    *   Carefully study all the provided "REFERENCE PYQ PDF and TXT" documents to understand question styles, common topics, difficulty level, and typical phrasing.
    *   **For General Awareness & Current Affairs:** Treat the provided TXT files as the definitive source of truth. All factual questions must be based solely on the information contained within these documents.
    *   **For Emotional Intelligence & Situational Judgment:** Use the events and contexts from the reference documents as inspiration to create relevant and realistic scenarios.
...
6.  **Content Rules:**
    ...
    *   For General Awareness or Emotional Quotient, please use one liner answer, with very simple explaination. 
...
function createQuestionSlide(pptx, question, directions, bgImagePath) {
    const slide = addSlideWithBackground(pptx, bgImagePath);
    slide.addText(`Question ${question.questionNumber}`, { x: 0.5, y: 0.4, w: '90%', fontSize: 11, fontFace: "Calibri", fontFace: 11,  bold: true, color: '1A365D' });
    ...
    if (directions) {
        ...
        slide.addText(`Directions: ${cleanDirections}`, {
            ...
            fontFace:'Calibri', fontSize: 11, italic: true, color: '555555', fill: { color: 'E2E8F0' }, margin: 10
        });
        currentY += 1.7;
    }
    ...
    question.options.forEach(opt => {
        ...
        if (opt.svg) {
            slide.addText(`${opt.label})`, { x: 0.75, y: currentY, w: 0.5, h: 0.5, fontSize: 14, fontFace: 'Calibri' });
            const base64Svg = svgToBase64(opt.svg);
            if (base64Svg) slide.addImage({ data: base64Svg, x: 1.25, y: currentY - 0.25, w: 1, h: 1 });
            currentY += 1.2;
        } else {
            slide.addText(optionText, { x: 0.75, y: currentY, w: '85%', h: 0.3, fontSize: 14, fontFace: 'Calibri' });
            currentY += 0.4;
        }
    });
}
...
function createAnswerSlide(pptx, question, bgImagePath) {
    const slide = addSlideWithBackground(pptx, bgImagePath);
    slide.addText(`Answer & Solution: Q${question.questionNumber}`, { x: 0.5, y: 0.4, w: '90%', fontSize: 11, bold: true, fontFace: 'Calibri', color: '1A365D' });
    ...
    slide.addText(explanationText, {
	    x: 0.5, y: 1.6, w: hasSvg ? '50%' : '90%', h: 3.8, fontSize: 11, fontFace:'Calibri'
    });
    if (hasSvg) {
...
async function filesToGenerativeParts(filePaths, label) {
    ...
    for (const filePath of filePaths) {
        // Get the file's extension to determine its type
        const extension = path.extname(filePath).toLowerCase();
        let mimeType;

        // STEP 1: Determine the MIME type (the file's "label") based on its extension.
        // This tells the API what kind of file it is receiving.
        switch (extension) {
            case '.pdf':
            ...
            case '.txt':
                ...
                break;
            default:
                // If the file type is not supported, log a warning and skip to the next file.
                console.warn(`  - Warning: Unsupported file type '${extension}' for ${path.basename(filePath)}. Skipping.`);
        ...
        try {
            ...
            if (fileSize > maxFileSize) {
                console.warn(`  - Warning: File ${path.basename(filePath)} exceeds the 20MB limit. Skipping.`);
                continue;
            }

            // STEP 2: Read the entire file into a raw data buffer.
            const fileBuffer = await fs.readFile(filePath);

            // STEP 3: Convert the raw buffer into a Base64 encoded string.
            // This is required by the API for all file types (both .pdf and .txt)
            // to ensure the data is transmitted safely within the JSON payload.
            const encodedData = fileBuffer.toString('base64');

            // Add the processed file to the parts array in the required format
            parts.push({
                inlineData: {
                    mimeType: mimeType, // The label from Step 1
                    data: encodedData,  // The safely encoded content from Step 3
                },
            });
        } catch (error) {
