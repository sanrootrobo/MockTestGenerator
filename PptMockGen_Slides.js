...
class GoogleSlidesManager {
    ...
    async initialize(credentialsPath = CREDENTIALS_PATH) {
        try {
            console.log('🔐 Initializing Google Slides API...');
    ...
    async getNewToken(oAuth2Client) {
        ...
        // For now, we'll use the authenticate function from google-cloud/local-auth
        try {
    ...
    async uploadPresentationFromFile(filePath, title) {
        try {
            console.log(`📤 Uploading ${path.basename(filePath)} to Google Drive...`);
            ...
            console.log(`✅ Uploaded to Google Drive: ${driveResponse.data.name}`);
            console.log(`🔗 View Link: ${driveResponse.data.webViewLink}`);

            return {
                fileId: driveResponse.data.id,
                name: driveResponse.data.name,
                webViewLink: driveResponse.data.webViewLink,
                exportLinks: driveResponse.data.exportLinks
            };
            
        } catch (error) {
            console.error(`❌ Failed to upload presentation: ${error.message}`);
            throw error;
        }
    }

    async createGoogleSlidesFromJson(jsonData, title) {
        try {
            ...
            // Get the presentation to access slide IDs
            ...
            // Title slide
            await this.createTitleSlide(presentationId, slides[slideIndex].objectId, jsonData);
            slideIndex++;
            
            // Instructions slide
            const instructionsSlideId = await this.addSlide(presentationId);
            await this.createInstructionsSlide(presentationId, instructionsSlideId, jsonData);
            slideIndex++;
            
            // Question slides
            const allQuestions = this.extractAllQuestions(jsonData);
            
            for (const question of allQuestions) {
                const questionSlideId = await this.addSlide(presentationId);
                await this.createQuestionSlide(presentationId, questionSlideId, question);
                slideIndex++;
            }
            
            // Answer slides divider
            const answerDividerSlideId = await this.addSlide(presentationId);
            await this.createAnswerDividerSlide(presentationId, answerDividerSlideId);
            slideIndex++;
            
            // Answer slides
            for (const question of allQuestions) {
                const answerSlideId = await this.addSlide(presentationId);
                await this.createAnswerSlide(presentationId, answerSlideId, question);
                slideIndex++;
            }
            
            const webViewLink = `https://docs.google.com/presentation/d/${presentationId}/edit`;
            console.log(`✅ Google Slides created successfully: ${title}`);
            ...
            console.error(`❌ Failed to create Google Slides: ${error.message}`);
            throw error;
        }
    }
    
    async addSlide(presentationId) {
        const requests = [{
            createSlide: {
                insertionIndex: 1000, // Insert at the end
            }
        }];
    ...
    async createTitleSlide(presentationId, slideId, jsonData) {
        const requests = [
            ...
            // Add subtitle with exam details
            {
                insertText: {
                    ...
                    insertionIndex: 0,
                    text: `Total Questions: ${jsonData.examDetails.totalQuestions} | Time: ${jsonData.examDetails.timeAllotted} | Max Marks: ${jsonData.examDetails.maxMarks}`
                }
            }
        ];
    ...
    async createInstructionsSlide(presentationId, slideId, jsonData) {
        const instructionsText = jsonData.instructions.title + '\n\n' + 
            jsonData.instructions.points.map(point => '• ' + point).join('\n');
    ...
    async createQuestionSlide(presentationId, slideId, question) {
        const questionText = `Question ${question.questionNumber}\n\n${question.questionText}\n\n` +
    ...
    async createAnswerDividerSlide(presentationId, slideId) {
        const requests = [
            {
                insertText: {
                    objectId: slideId + '_content',
    ...
    async createAnswerSlide(presentationId, slideId, question) {
        const answerText = `Answer & Solution: Q${question.questionNumber}\n\n` +
...
class LibreOfficeConverter {
    ...
    detectLibreOfficePath() {
        ...
        // You might want to add logic to detect which one exists
        // For now, defaulting to macOS path as per your example
        return '/Applications/LibreOffice.app/Contents/MacOS/soffice';
    }

    async convertPptxToPpt(inputPath, outputDir = null) {
        try {
            const inputDir = path.dirname(inputPath);
            const workingDir = outputDir || inputDir;
            const inputFilename = path.basename(inputPath);
            
            console.log(`🔄 Converting ${inputFilename} to PPT format...`);
            
            // LibreOffice command to convert PPTX to PPT
            const command = `"${this.libreOfficePath}" --headless --convert-to ppt --outdir "${workingDir}" "${inputPath}"`;
            ...
            const outputPath = path.join(workingDir, `${baseName}.ppt`);
            
            // Verify the file was created
            try {
                await fs.access(outputPath);
                console.log(`✅ PPT conversion successful: ${path.basename(outputPath)}`);
                return outputPath;
            } catch (accessError) {
            ...
            if (error.message.includes('ENOENT') || error.message.includes('not found')) {
                console.error('💡 LibreOffice not found. Please install LibreOffice or update the path.');
                console.error('   Download from: https://www.libreoffice.org/download/');
            } else if (error.message.includes('timeout')) {
                console.error('💡 Conversion timed out. The file might be too large or LibreOffice is busy.');
            }
    ...
    async convertAllPptxInDirectory(directoryPath) {
        try {
            const files = await fs.readdir(directoryPath);
            const pptxFiles = files.filter(file => path.extname(file).toLowerCase() === '.pptx');
            
            console.log(`📁 Found ${pptxFiles.length} PPTX files to convert in ${directoryPath}`);
            
            const results = [];
            for (const pptxFile of pptxFiles) {
                const inputPath = path.join(directoryPath, pptxFile);
                try {
                    const outputPath = await this.convertPptxToPpt(inputPath);
                    results.push({
                        success: true,
                        inputPath,
                        outputPath,
            ...
            console.log(`🔄 LibreOffice Conversion Summary:`);
            console.log(`✅ Successfully converted: ${successful.length}/${pptxFiles.length}`);
            console.log(`❌ Failed conversions: ${failed.length}/${pptxFiles.length}`);
            
            if (successful.length > 0) {
                console.log(`📁 Converted PPT files:`);
    ...
    setLibreOfficePath(customPath) {
        this.libreOfficePath = customPath;
        console.log(`🔧 LibreOffice path set to: ${customPath}`);
    }
}
...
// --- JSON TO PPTX CONVERSION (existing functions unchanged) ---
...
// --- PDF GENERATION FROM HTML (unchanged) ---
...
// --- MAIN MOCK GENERATION FUNCTION (enhanced with PPT conversion and Google Slides) ---
async function generateSingleMock(contents, outputPath, mockNumber, totalMocks, options, libreOfficeConverter, googleSlidesManager) {
    const maxRetries = 3;
    let lastError = null;
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            ...
            // Generate PowerPoint files if requested
            let pptxPath = null;
            let pptPath = null;
            let googleSlidesResult = null;
            
            if (options.ppt || options.convertToPpt || options.googleSlides) {
                pptxPath = generateOutputFilename(options.output, mockNumber, totalMocks, '.pptx');
                const backgroundPath = options.pptBackground || null;
                await generatePptFromJson(jsonData, pptxPath, backgroundPath);
                
                // Convert PPTX to PPT if requested
                if ((options.convertToPpt || options.googleSlides) && libreOfficeConverter) {
                    try {
                        console.log(`🔄 Converting PPTX to PPT for mock ${mockNumber}...`);
                        pptPath = await libreOfficeConverter.convertPptxToPpt(pptxPath);
                    } catch (conversionError) {
                        console.warn(`⚠️  PPT conversion failed for mock ${mockNumber}: ${conversionError.message}`);
                        // Continue without PPT conversion
                    }
                }
                
                // Upload to Google Slides if requested
                if (options.googleSlides && googleSlidesManager) {
                    try {
                        const mockTitle = `${jsonData.examTitle} - Mock ${mockNumber}`;
                        
                        if (options.uploadMethod === 'direct' && pptxPath) {
                            // Upload PPTX file to Google Drive
                            console.log(`📤 Uploading PPTX to Google Drive for mock ${mockNumber}...`);
                            googleSlidesResult = await googleSlidesManager.uploadPresentationFromFile(
                                pptxPath, 
                                mockTitle
                            );
                        } else {
                            // Create Google Slides directly from JSON
                            console.log(`🎯 Creating Google Slides directly for mock ${mockNumber}...`);
                            googleSlidesResult = await googleSlidesManager.createGoogleSlidesFromJson(
                                jsonData, 
                                mockTitle
                            );
                        }
                    } catch (slidesError) {
                        console.warn(`⚠️  Google Slides creation failed for mock ${mockNumber}: ${slidesError.message}`);
                        // Continue without Google Slides
                    }
                }
            }
            ...
            // Add additional file paths to result
            if (pptxPath) result.pptxPath = pptxPath;
            if (pptPath) result.pptPath = pptPath;
            if (googleSlidesResult) result.googleSlides = googleSlidesResult;
...
async function main() {
    program
        ...
        .option("--convert-to-ppt", "Convert generated PPTX files to PPT format using LibreOffice")
        .option("--libreoffice-path <path>", "Custom path to LibreOffice soffice executable")
        .option("--google-slides", "Create Google Slides presentations")
        .option("--google-credentials <file>", "Path to Google OAuth2 credentials JSON file (default: credentials.json)")
        .option("--upload-method <method>", "Google Slides upload method: 'direct' (upload PPTX) or 'create' (create from JSON)", "create")
    ...
    // Initialize LibreOffice converter if needed
    let libreOfficeConverter = null;
    if (options.convertToPpt || options.googleSlides) {
        libreOfficeConverter = new LibreOfficeConverter();
        if (options.libreOfficePath) {
            libreOfficeConverter.setLibreOfficePath(options.libreOfficePath);
        }
        console.log('🔧 LibreOffice converter initialized for PPT conversion.');
    }

    // Initialize Google Slides manager if needed
    let googleSlidesManager = null;
    if (options.googleSlides) {
        try {
            await import('googleapis');
            await import('@google-cloud/local-auth');
            await import('google-auth-library');
            console.log('✅ Google APIs available - Google Slides integration is enabled.');
            
            googleSlidesManager = new GoogleSlidesManager();
            const credentialsPath = options.googleCredentials || CREDENTIALS_PATH;
            const initialized = await googleSlidesManager.initialize(credentialsPath);
            
            if (!initialized) {
                console.error('❌ Failed to initialize Google Slides API. Continuing without Google Slides integration.');
                googleSlidesManager = null;
            }
        } catch (error) {
            console.error('❌ Google APIs not available. Please install required packages:');
            console.error('npm install googleapis @google-cloud/local-auth google-auth-library');
            console.error('Continuing without Google Slides integration.');
            googleSlidesManager = null;
        }
    }
    ...
    try {
        ...
        if (options.convertToPpt) outputFormats.push("PPT");
        if (options.googleSlides) outputFormats.push("Google Slides");
        ...
        for (let i = 1; i <= numberOfMocks; i++) {
            ...
            generationTasks.push(() => generateSingleMock(
                contents, 
                outputPath, 
                i, 
                numberOfMocks, 
        ...
        if (successful.length > 0) {
            ...
            successful.sort((a,b) => a.mockNumber - b.mockNumber).forEach(mockResult => {
                console.log(`\n  Mock ${mockResult.mockNumber}:`);
                console.log(`    📄 PDF: ${path.basename(mockResult.outputPath)} (${mockResult.contentLength} chars, API Key ${mockResult.keyIndex + 1})`);
                
                if (mockResult.pptxPath) {
                    console.log(`    📊 PPTX: ${path.basename(mockResult.pptxPath)}`);
                }
                
                if (mockResult.pptPath) {
                    console.log(`    📊 PPT: ${path.basename(mockResult.pptPath)}`);
                }
                
                if (mockResult.googleSlides) {
                    console.log(`    🌐 Google Slides: ${mockResult.googleSlides.title}`);
                    console.log(`       🔗 ${mockResult.googleSlides.webViewLink}`);
                }
                
                if (options.saveJson) {
                    const debugJsonPath = generateDebugJsonFilename(options.output, mockResult.mockNumber, numberOfMocks);
                    console.log(`    💾 JSON: ${path.basename(debugJsonPath)}`);
                }
                
                if (options.saveHtml) {
                    const debugHtmlPath = generateDebugHtmlFilename(options.output, mockResult.mockNumber, numberOfMocks);
                    console.log(`    💾 HTML: ${path.basename(debugHtmlPath)}`);
                }
            });

            // Google Slides Summary
            const googleSlidesResults = successful.filter(r => r.googleSlides);
            if (googleSlidesResults.length > 0) {
                console.log(`\n🌐 Google Slides Summary:`);
                googleSlidesResults.forEach(result => {
                    console.log(`  📊 ${result.googleSlides.title}`);
                    console.log(`     🔗 ${result.googleSlides.webViewLink}`);
                });
            }
        }
        ...
        if (options.convertToPpt && libreOfficeConverter && successful.length > 0) {
            console.log(`\n🔄 Post-processing: Converting remaining PPTX files to PPT...`);
            const outputDir = path.dirname(options.output);
            try {
                await libreOfficeConverter.convertAllPptxInDirectory(outputDir);
            } catch (error) {
                console.warn(`⚠️  Batch PPT conversion failed: ${error.message}`);
            }
        }
        ...
        // Setup instructions for Google Slides
        if (options.googleSlides && !googleSlidesManager) {
            ...
            console.log(`3. Enable the Google Slides API and Google Drive API`);
            console.log(`4. Create OAuth 2.0 credentials (Desktop application)`);
            console.log(`5. Download the credentials JSON file and save it as 'credentials.json'`);
            console.log(`6. Run the script again with --google-slides flag`);
        }
